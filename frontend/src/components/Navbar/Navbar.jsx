...
<Link to = "/" className = "flex items-center">
    <img className = "h-10 me-4"
     src = "https://cdn-icons-png.flaticon.com/128/10433049.png"
    alt = "logo"/>
    <h1 className = "text-2xl-font-semibold">BookHeaven
        ...
        <div className = "nav-links-bookheaven flex items-center gap-4">
         ...
         <div className = {`${MobileNav}"bg-zinc-800 h-screen absolute top-0 left-0 w-full z-40 flex-col items-center justify-center`}>
         {links.map((items,i)=>(
            <Link to = {items.link}
...
<Link to = "LogIn" className={` ${MobileNav} px-8 mb-8 text-3xl font-semibold py-2 border border-blue-500 rounded text-white hover:bg-white hover:text-zinc-800 transition-all duration-300`}>Login</Link>
                <Link to = "SignUp" className = {` ${MobileNav} px-8 mb-8 text-3xl font-semibold py-2 border border-blue-500 rounded text-white hover:bg-white hover:text-zinc-800 transition-all duration-300`}>SignUp</Link>
